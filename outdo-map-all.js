...
// Free, keyless basemap: Esri light grey canvas, with OpenStreetMap as fallback
var tiles = L.tileLayer('https://services.arcgisonline.com/ArcGIS/rest/services/Canvas/World_Light_Gray_Base/MapServer/tile/{z}/{y}/{x}',
  { maxZoom:16, attribution:'Tiles &copy; Esri' }).addTo(map);
// Place-name labels (Esri reference layer) in their own pane, above the routes
map.createPane('onmLabels');
map.getPane('onmLabels').style.zIndex = 650;
map.getPane('onmLabels').style.pointerEvents = 'none';
var labels = L.tileLayer('https://services.arcgisonline.com/ArcGIS/rest/services/Canvas/World_Light_Gray_Reference/MapServer/tile/{z}/{y}/{x}',
  { maxZoom:16, pane:'onmLabels' }).addTo(map);
var fellBack = false;
...
// Recalculate size once the container is laid out / resized (Webflow columns size late)
function refit(){ map.invalidateSize(); if(!state.sel) map.fitBounds(home,{padding:[24,24]}); }
setTimeout(refit,200); setTimeout(refit,600); setTimeout(refit,1200); window.addEventListener('resize',refit);
if('ResizeObserver' in window){ new ResizeObserver(refit).observe(document.getElementById('onm-map')); }
...
function fillWebflowForm(){
  // Push current selection into any form fields on the host page
  var g = cur();
  var vals = {
    'Selected-Route': g ? summary(g) : (state.filter==='All' ? 'Whole NCT network' : 'All '+state.filter+' Nottingham routes'),
    'Route-Area': g ? g.area+' Nottingham' : (state.filter==='All' ? 'All areas' : state.filter+' Nottingham'),
  ...
  var txt = g ? summary(g) : (state.filter==='All' ? 'Whole NCT network' : 'All '+state.filter+' Nottingham routes');
...
function renderInfo(){
  ...
  fillWebflowForm();
  var cta = $('onm-open');
  if(cta) cta.textContent = g ? (g.colorName==='Pathfinder' ? 'Advertise on Pathfinder' : 'Advertise on the '+g.colorName) : 'Advertise on the whole NCT network';
  if(!el) return;
  if(!g){
...
// --- Modal ---
function openModal(){
  // Re-parent overlay to <body>: a Webflow ancestor with transform/filter traps position:fixed
  var ovl = $('onm-ovl');
  if(ovl.parentNode !== document.body) document.body.appendChild(ovl);
  var g = cur();
  var title = document.getElementById('onm-mtitle');
  if(title) title.textContent = g ? 'Advertise on this network' : 'Advertise on the whole NCT network';
  $('onm-ctx').style.display = g ? '' : 'none';
  $('onm-ctx').innerHTML = (g?'<span class="onm-dot" style="width:12px;height:12px;background:'+g.color+'"></span>':'')+
    '<span>'+esc(summary(g))+'</span>';
...
if($('onm-open') && $('onm-ovl')) $('onm-open').addEventListener('click',openModal);
if($('onm-close')) $('onm-close').addEventListener('click',closeModal);
if($('onm-close2')) $('onm-close2').addEventListener('click',closeModal);
if($('onm-ovl')) $('onm-ovl').addEventListener('click',function(e){ if(e.target===this) closeModal(); });

if($('onm-form')) $('onm-form').addEventListener('submit',function(e){
  e.preventDefault();
